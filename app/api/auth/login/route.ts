...
export async function POST(req:Request){
 try{
  ...
  const account=accounts.find(x=>x.username&&x.password&&x.token&&username===x.username&&password===x.password);
  if(!account)return Response.json({error:'账号或密码错误'},{status:401});
