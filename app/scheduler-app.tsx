...
type AppState={workers:Worker[],parts:Part[],orders:Order[],allocations:Allocation[],lastRun:string};
...
export default function SchedulerApp(){
 ...
 async function importExcel(file:File){const buf=await file.arrayBuffer(),book=XLSX.read(buf),rows=XLSX.utils.sheet_to_json<Record<string,unknown>>(book.Sheets[book.SheetNames[0]]);const imported=rows.map((r,i)=>{const code=String(r['物料编码']??r['物料']??'');const p=data.parts.find(x=>x.code===code);return{id:crypto.randomUUID(),orderNo:String(r['订单号']??r['生产订单']??`MES-${Date.now()}-${i+1}`),customer:String(r['客户']??r['使用单位']??''),partCode:code,name:String(r['名称']??p?.name??''),qty:Number(r['数量']??0),done:Number(r['已完成数量']??0),due:String(r['交期']??r['计划完成']??''),priority:Number(r['优先级']??3),status:'待排'} as Order}).filter(o=>o.partCode);await persist({...data,orders:[...data.orders,...imported]},`已从MES文件导入 ${imported.length} 条订单`)}
