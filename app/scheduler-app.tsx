...
export default function SchedulerApp(){
 ...
 const [resolving,setResolving]=useState<Order|null>(null),[resolveForm,setResolveForm]=useState({unit:'',category:'',workers:[] as string[]});
 ...
 async function confirmOrder(order:Order){const part=data.parts.find(p=>p.code===order.partCode),missing=[];if(!part?.unit)missing.push('单件定额');if(!part||!matchingWorkers(part,data.workers).length)missing.push('可加工人员');if(!order.due)missing.push('交期');if(order.qty<=0)missing.push('数量');if(missing.length){setNotice(`暂不能确认：请先补充${missing.join('、')}`);setTimeout(()=>setNotice(''),2600);return}const issues=(data.issues||[]).filter(x=>x.key!==order.id),next={...data,issues};const allocations=schedule(next,false);await persist({...next,allocations,lastRun:new Date().toLocaleString('zh-CN')},`已确认 ${order.orderNo}，并更新排产`)}
 function startResolve(order:Order){const part=data.parts.find(p=>p.code===order.partCode);setResolving(order);setResolveForm({unit:part?.unit?String(part.unit):'',category:part?.category||order.name,workers:part?.workers||[]})}
 async function saveResolution(e:React.FormEvent){e.preventDefault();if(!resolving)return;const existing=data.parts.find(p=>p.code===resolving.partCode),part:Part={id:existing?.id||crypto.randomUUID(),code:resolving.partCode,name:resolving.name,category:resolveForm.category,unit:Number(resolveForm.unit)||0,workers:resolveForm.workers};const parts=[...data.parts.filter(p=>p.code!==part.code),part],missing=[];if(!part.unit)missing.push('单件定额');if(!matchingWorkers(part,data.workers).length)missing.push('可加工人员');if(missing.length){setNotice(`仍需补充：${missing.join('、')}`);setTimeout(()=>setNotice(''),2600);return}const issues=(data.issues||[]).filter(x=>x.key!==resolving.id),next={...data,parts,issues};const allocations=schedule(next,false);await persist({...next,allocations,lastRun:new Date().toLocaleString('zh-CN')},`${resolving.partCode} 已确认并加入排产`);setResolving(null)}
 ...
 function addPart(e:React.FormEvent){e.preventDefault();if(!partForm.code)return;const existing=data.parts.find(p=>p.code===partForm.code),p:Part={id:existing?.id||crypto.randomUUID(),code:partForm.code,name:partForm.name,category:partForm.category,unit:+partForm.unit||0,workers:partForm.workers};persist({...data,parts:[...data.parts.filter(x=>x.code!==p.code),p]},existing?'工件资料已更新':'工件资料已新增');setPartForm({code:'',name:'',category:'',unit:'',workers:[]})}
 ...
 async function importExcel(file:File){
  ...
  if(firstRows.some(r=>'计划编号' in r&&'关联订单' in r&&'物料编码' in r)){
   ...
   for(const row of firstRows){const code=clean(row['物料编码']),planNo=clean(row['计划编号']),related=clean(row['关联订单']);if(!code||!planNo)continue;const name=clean(row['名称']),due=excelDate(row['计划完成']),qty=Number(row['数量'])||0,id=stableId('xingqiyun',planNo,related,code),orderNo=`XQY-${planNo}-${related||'无订单'}-${code}`,part=data.parts.find(p=>p.code===code);
    ...
    const reasons=[];if(!due)reasons.push('缺少交期');if(!qty)reasons.push('数量无效');if(!part?.unit)reasons.push('缺少单件定额');if(!part||!matchingWorkers(part,data.workers).length)reasons.push('无匹配人员');if(reasons.length)issues.push({key:id,partCode:code,name,reason:reasons.join('、')});
   }
 ...
 const tabs=['看板','订单管理','人员产能','工件定额','排产结果','历史记录'];
