  ...
  {tab==='订单管理'&&<div className="split">
   <section className="panel">
    ...
    <table><thead><tr><th>优先级</th><th>订单号</th><th>客户</th><th>工件</th><th>数量</th><th>已完成</th><th>交期</th><th>状态</th><th>操作</th></tr></thead>
     <tbody>{data.orders.map(o=>{const issue=data.issues?.some(x=>x.key===o.id);return <tr key={o.id}><td><span className={`priority p${o.priority}`}>{o.priority}</span></td><td className="mono">{o.orderNo}</td><td>{o.customer}</td><td>{o.name}</td><td>{o.qty}</td><td>{o.done}</td><td>{o.due}</td><td>{issue?<span className="late">待确认</span>:<span className="pending">{o.status}</span>}</td><td><button className={issue?'late':'ok'} style={{border:0,cursor:'pointer'}} onClick={()=>confirmOrder(o)}>{issue?'确认资料':'确认并重排'}</button></td></tr>})}</tbody>
    </table>
   </section>
