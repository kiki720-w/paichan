...
export default function SchedulerApp(){
 ...
 return <div className="shell">
  ...
  <main><header><div><p>生产计划中心</p><h1>{tab}</h1></div><div className="header-actions"><button className="import" onClick={refreshData}>↻ 刷新数据</button><button className="import" onClick={clearSchedule} style={{color:'#a8322b'}}>清空排产</button><label className="import">导入兴企云 / Excel<input type="file" accept=".xlsx,.xls,.csv" onChange={e=>e.target.files?.[0]&&importExcel(e.target.files[0])}/></label><button className="primary" onClick={run}>▶ 自动排产</button></div></header>
  ...
  {tab==='排产结果'&&<section className="panel"><div className="panel-title"><div><small>按人员与日期展开</small><h3>每日派工计划</h3></div><div className="legend"><i className="green"/>按期 <i className="red"/>延期</div></div>{data.allocations.length?<GroupedSchedule rows={data.allocations}/>:<div className="empty"><b>尚未生成计划</b><span>点击右上角“自动排产”开始计算。</span></div>}</section>}
  </main>
 </div>
}
