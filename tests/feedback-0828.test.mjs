...
test('order table provides persistent bulk assignment controls',()=>{assert.match(source,/批量指定加工人员/);assert.match(source,/toggleVisibleOrders/);assert.match(source,/saveBulkAssignment/);assert.match(source,/仅下次排产生效/);assert.match(source,/setSelectedOrderIds\(\[\]\)/)});
test('hour-tier rules are configurable and no fixed customer example remains',()=>{assert.match(source,/工件定额','业务规则','排产结果/);assert.match(source,/工时分档规则/);assert.match(source,/产能不足允许候补/);assert.doesNotMatch(source,/工时 &gt; 35 由王超伟/)});
